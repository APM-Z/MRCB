...
#define FREQ8       1.191795E9          /* E5a+b B2(B2a+B2b) frequency (Hz) */
...
#define FREQC_CMP   1.57542E9           /* BeiDou B1C frequency (Hz) */
#define FREQA_CMP   1.17645E9           /* BeiDou B2A frequency (Hz) */
#define MAXCELLS 96
...
#define NSDIA      (NFREQ>2?NFREQ*2-1:NFREQ+1) /* single satellite dia detect */
...
#define MAXSAT      (NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATIRN+NSATLEO) /* max satellite number (1 to MAXSAT) */
#define MAXRCV      1
#define MAXOBS      320                 /* max number of obs in an epoch */
...
#define IMUDTTOL    0.010               /* tolerance of time difference (s) */
...
#define MAXSTRRTK   4                   /* max number of stream in RTK server */
#define MAXSOLMSG   8191                /* max length of solution message */
#define MAXRAWLEN   20480               /* max length of receiver raw message */
...
#define CODE_L8L    37                  /* obs code: E5(a+b)I   (GAL) */
...
#define MAXCODE     66                  /* max number of obs code */
...
#define SOLF_INS    6                   /* solution format: GSI F1/F2 */
...
#define POSOPT_FIXED 3                  /* pos option: self definition*/
#define POSOPT_RINEX 4                  /* pos option: rinex header pos */
#define POSOPT_RTCM  5                  /* pos option: rtcm station pos */
#define POSOPT_RAW   6                  /* pos option: raw station pos */
...
#define STR_WiFi     15                 /* stream type: ntrip by wifi */
...
#define STRFMT_SINbin  23               /* stream format: Sino cnb */
#define STRFMT_UniCore 24               /* stream format: Unicore */
...
#define NF(opt)       NFREQ
#define NP(opt)       MAXSAT             /* frequency-independent parameters */  
#define NI(opt)       MAXSAT		     /* ionospheric */
#define NM(opt)       (NSYS*NFREQ)       /* receiver code bias*/
#define NN(opt)       (NFREQ*MAXSAT)     /* satellite code bias*/
#define NIFB(opt)     NFREQ
#define NA(opt)       (NFREQ*MAXSAT)     /* ambiguity */
#define NX(opt)       (NP(opt)+NI(opt)+NM(opt)+NN(opt)+NIFB(opt)+NA(opt))
#define II(opt,s)     (NP(opt)+(s)-1) /*Ion*/
#define IM(opt,s,f)   (NP(opt)+NI(opt)+NFREQ*s+f)     /*rcb*/
#define SCBN(opt,s,f) (NP(opt)+NI(opt)+NM(opt)+MAXSAT*f+(s)-1)         /* scb */
#define IFB(opt,f)    (NP(opt)+NI(opt)+NM(opt)+NN(opt)+f)
#define IA(opt,s,f)   (NP(opt)+NI(opt)+NM(opt)+NN(opt)+NIFB(opt)+MAXSAT*(f)+(s)-1) /* amb */
...
typedef struct {        /* observation data record */
    ...
    double EP[NFREQ+NEXOBS][TYPEN]; /* extend observation data pseudorange (m) */
    double EL[NFREQ+NEXOBS][TYPEN]; /* extend observation data pseudorange (m) */
} obsd_t;
...
typedef struct {
    ...
    int    iode;
    double var;			/* sat position and clock error variances (m^2) */
    double rs[6];		/* satellite positions and velocities (ecef) */
    double dts[3];		/* satellite clocks, satellite clocks velocities, relativistic effect correction */
...
typedef struct {               /* antenna parameter type */
	...
	gtime_t ts,te;             /*valid time start and end*/
	double off[NFREQ][3];      /*phase center offset e/n/u or x/y/z (m)*/
	double var[NFREQ][73][19]; /* phase center variation (m) */
} _pcv_t;

typedef struct {               /* antenna parameter type */
	int n,nmax;                /* number of data/allocated */
	_pcv_t *pcv;               /* antenna parameters data */
} pcvr_t;
...
typedef struct {        /* navigation data type */
    ...
    double rsun[3];  
    double rmoon[3]; 
    double erpv[5]; //erp values {xp,yp,ut1_utc,lod} (rad,rad,s,s/d)
} nav_t;
...
typedef struct {        /* station parameter type */
	...
	_pcv_t pcvr;        /* receiver antenna parameters*/
} sta_t;

typedef struct {        /* solution type */
    gtime_t time;       /* time (GPST) */
	gtime_t time_filter;/* time of last successful filtering */
    unsigned char stat,pstat; /* solution status (SOLQ_???) */
    int  ns;                /* number of valid satellites */
    int  sys_ns[NSYS];      /* Number of Satellites per System */
    int redundancy;     /* the redundancy */
    unsigned char ista; /* ins update status (INSS_???) */
	unsigned char nfix; /* number of fixed ambiguities */
	unsigned char na;   /* number of all ambiguities */
    float age;          /* age of differential (s) */
    float ratio;        /* AR ratio factor for valiation */
    float thres;        /* AR ratio threshold for valiation */
    double fr[9];       /* ppp float postion */
                        /* {x,y,z,vx,vy,vz} or {e,n,u,ve,vn,vu} */
    double rr[9];       /* SPP position/velocity (m|m/s) */
                        /* {x,y,z,vx,vy,vz} or {e,n,u,ve,vn,vu} */
    float  qr[6];       /* position variance/covariance (m^2) */
                        /* {c_xx,c_yy,c_zz,c_xy,c_yz,c_zx} */
    double dop[5];      /* GDOP PDOP HDOP VDOP ADOP */
    int    rsat[NSYS];
	double dtr[NSYS];       /* spp receiver clock bias  every system (s) */
	double dts[MAXSAT];     /* satellite clock bias */
    
    double prex[MAXSAT];    //frequency-independent parameters
    double trop;        
    double ion[MAXSAT]; 
	double amb[MAXSAT][NFREQ]; 
	double resc[MAXSAT][NFREQ]; 
	double resp[MAXSAT][NFREQ]; 
    double rcb[NSYS][NFREQ]; 
    double scb[MAXSAT][NFREQ]; 
    double IFB[NFREQ];   /*GLONASS IFB*/
} sol_t;

typedef struct {           /* SNR mask type */
    int    ena[2];         /* enable flag {rover,base} */
    double mask[NFREQ][9]; /* mask (dBHz) at 5,15,...85 deg */
} snrmask_t;

typedef struct {           /* processing options type */
    char Initial;          /* 0->first epoch */
    char estm;             /* [estm], s */
    unsigned char exsats[MAXSAT]; /* excluded satellites (1:excluded,2:included) */
    int ratio;                    /* 1: close, 0: open */
    int navsys;                   /* navigation system */
    int sateph;                   /* satellite ephemeris/clock (EPHOPT_???) */
    int ionoopt;                  /* ionosphere option (IONOOPT_???) iwopt=1->ionospheric weight */
    ...
    double prn[6];               /* process-noise std [0]ambiguty,[1]iono,[2]trop,[3]acch,[4]accv,[5]pos */
    double sclkstab;             /* satellite clock stability (sec/sec) */
    double thresslip;            /* slip threshold of geometry-free phase (m) */
    double maxtdiff;             /* max difference of time (sec) */
    double maxinno;              /* reject threshold of innovation (m) */
    double antdel[MAXRCV][3];    /* antenna delta {e,n,u} */
    snrmask_t snrmask;           /* SNR mask */
    int nf;                      /* number of frequencies*/
    int soltype;                 /* solution type (0:forward) */
    int CONTINUE;                /* continue day process */
    char staname[5];             /* Station Name */
    //char AntType[21];            /* Antenna Type */
    int bds2;                    /* use bds2? */
    char DOY[10];                /* DOY date */
} prcopt_t;
...
typedef struct {        
	...
	char antinfo[MAXSTRPATH]; /* ant information */
    char solstat[MAXSTRPATH]; /* solution statistics file */
    char nav[MAXSTRPATH];     /* input nav file */
    char obs[MAXSTRPATH];     /* input obs file */
    char NavDir[MAXSTRPATH];       /* Batch nav file dir */
    char ObsDir[MAXSTRPATH];       /* Batch obs file dir */
    char outFile_PRO[MAXSTRPATH]; /*STAT PATH*/
    char outFile_OUT[MAXSTRPATH];  /*OUT  PATH*/
    char outFile_LOG[MAXSTRPATH];  /*OUT  PATH*/
} filopt_t;
...
typedef struct {
    ...
    double  amb[NFREQ];         /* float ambiguity(cycle)*/
    double  fixed_amb[NFREQ];   /* fixed ambiguity(cycle)*/
	double azel[2];       /* azimuth/elevation angles {az,el} (rad) */
...
typedef struct {            /* control/result type */
    unsigned char *F;      
    int rsat[NSYS];         /* satellite number used as the reference */
    int cvsat[MAXSAT]; 
    int basis_freq[NSYS];   /* frequency number of the basis of receiver code bias */
    ...
    double TqLOM;
    sol_t  sol;                 /*  solution */
    ssat_t ssat[MAXRCV*MAXSAT]; /* satellite status */
...
/* global variables */
extern const double diacri[];        /* critical value for statistical hypothesis testing: IA=1,alda0=0.001,power=0.8 */
...
EXPORT int  num2sys(int nsys);
EXPORT int  sys2num(int sys);
...
EXPORT int  normv4(const double *a,double *b);
...
EXPORT int lsq_(const double *A,const double *P,const double *y,int n,int m,double *x,double *Q,double *v);
...
EXPORT void    time2str3(gtime_t t, char* s, int n);
...
EXPORT void uniqnav(nav_t *nav, const prcopt_t* popt);
...
/* point models --------------------------------------------------------*/
EXPORT double satwavelen(int sat,int frq,const nav_t *nav, const prcopt_t* popt);
...
EXPORT double ionmodel(gtime_t t,const double *ion,const double *pos,const double *azel, const prcopt_t* opt);
...
EXPORT double time_doy(gtime_t t,int *year);
EXPORT gtime_t yrdoy2time(const int yyyy, const int doy);
extern double time2sod(gtime_t t);
EXPORT double julday(double y,double m,double d,double h);
EXPORT void   GMF(double dmjd,const double *pos,double zd,double *gmf);
EXPORT void   UNB3M(const double *pos,double DAYOYEAR,double *TZD);

/* antenna models ------------------------------------------------------------*/
EXPORT int readpcv(const char *file,pcvs_t *pcvs, prcopt_t* popt);
EXPORT int readpcvr(const char *file,const char *anttype,pcvr_t *pcvr);
EXPORT pcv_t  *searchpcv (int sat,gtime_t time, const pcvs_t *pcvs);
EXPORT void antmodel(const _pcv_t *pcv,const double *del,const double *azel,int opt,double *dant);
...
EXPORT void satantoff(const double *rs,const double *rsun,const pcv_t *pcv,double *dant);
EXPORT int  satposs(gtime_t teph,const prcopt_t*opt,const obsd_t *obs,int n,const nav_t *nav,int ephopt,rscs_t *rscs);
...
/* Data Process ------------------------------------------------------*/
EXPORT int pntpos(const obsd_t *obs,int n,const nav_t *nav,const prcopt_t *opt, sol_t *sol,rscs_t *rscs,ssat_t *ssat,char *msg);
EXPORT int calrcb(FILE**fp,rcb_t*rcb,obsd_t*obs,int n, nav_t *nav,sta_t*sta, const solopt_t* sopt);
EXPORT int RCB (FILE**fp,rcb_t *rcb,const obsd_t *obs, int n,const nav_t *nav,rscs_t *rscs,sta_t*sta, const solopt_t* sopt);
EXPORT int rtknx(const prcopt_t *opt);
...
EXPORT int rcb_uncompress(const char *file, char *uncfile);


#define MAXOBSFILE 60
#define MAXPROFILE 60
#define NIfile     2

typedef struct {
    char *obsfile[MAXOBSFILE]; //O
    char *rnxfile[MAXPROFILE]; //N
    int idx[NIfile]; 
} Ifile;
...
/*extend functions*/
extern int readrnxh(FILE *fp, double *ver, char *type, int *sys, int *tsys,char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta);
extern int readrnxnav(FILE *fp, const char *opt, double ver, int sys,nav_t *nav);
extern int readrnxobs(FILE *fp, gtime_t ts, gtime_t te, double tint,const char *opt, int rcv, double ver, int *tsys,char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta, const prcopt_t* popt);
