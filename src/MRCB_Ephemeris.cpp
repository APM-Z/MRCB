...
extern double eph2pos(gtime_t time,const eph_t *eph,double *rs,double *dts,double *var)
{
    ...
    double xg,yg,zg,sino,coso,rel=0;
    int n,sys,prn;
    ...
    if (n>=MAX_ITER_KEPLER) return rel;
    ...
    *dts=eph->f0+eph->f1*tk+eph->f2*tk*tk;
    ...
    if(eph->f0!=0.0) rel=-2.0*sqrt(mu*eph->A)*eph->e*sinE/SQR(CLIGHT); 
    *dts+=rel;
                  ...
                  int iode,double *rs,double *dts,double *var,int *svh)
{
    ...
    if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP) {
        ...
        dts[2] = eph2pos(time, eph, rs, dts, var);
...
static int satpos(gtime_t time,gtime_t teph,int sat,int ephopt,const nav_t *nav,
                  double *rs,double *dts,double *var,int iode,int *svh)
{
    if      (ephopt==EPHOPT_BRDC) {
        return ephpos(time,teph,sat,nav,iode,rs,dts,var,svh);
    }
    return 0;
}
/* satellite positions and clocks */
extern int satposs(gtime_t teph,const prcopt_t*opt,const obsd_t *obs,int n,const nav_t *nav,
                   int ephopt,rscs_t *rscs)
{
    gtime_t time;
    double dt,pr;
    int i,j,sat,ns=0;
    ...
    for (i=0;i<n&&i<MAXRCV*MAXOBS;i++) {
        ...
        if (!ephclk(time,teph,obs[i].sat,nav,&dt)) continue;
        time=timeadd(time,-dt);
        /* satellite position and clock at transmission time */
        //satellite clock does not include code bias correction
        if (!satpos(time,teph,obs[i].sat,ephopt,nav,rscs[i].rs,rscs[i].dts,&rscs[i].var,rscs[i].iode,NULL)) continue;
        /* if no precise clock available, use broadcast clock instead */
        if (rscs[i].dts[0]==0.0) continue;
        rscs[i].svh=0;
        ++ns;
    }
    return ns;
